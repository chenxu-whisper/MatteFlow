...
def test_green_screen_effect_reconstruction_preserves_cyan_halo_near_lightning_core():
    matte = HybridMatte(MattingConfig(use_ai=False))
    matte.last_active_ai_model = "gvm"
    matte.last_fallback_quality_metrics = {
        "score_blocked": True,
    ...
    assert float(merged[luminous_core].mean()) >= 0.70
    assert float((merged[near_lightning_cyan_halo] > 0.03).mean()) >= 0.90
    assert float(merged[near_lightning_cyan_halo].mean()) >= 0.20
...
def test_green_screen_real_frame_3_independent_teal_background_is_not_effect_owned():
    matte = HybridMatte(MattingConfig(use_ai=False))
    matte.last_active_ai_model = "gvm"
    matte.last_fallback_quality_metrics = {
        "score_blocked": True,
    ...
    assert debug is not None
    assert int(independent_teal_background.sum()) >= 50_000
    assert float(debug["background_evidence"][independent_teal_background].mean()) >= 0.99
    assert float((merged[independent_teal_background] > 0.05).mean()) <= 0.03
    assert float((merged[independent_teal_background] > 0.20).mean()) <= 0.005
    assert float(debug["ownership_effect"][independent_teal_background].mean()) <= 0.02
    assert float(debug["ownership_background"][independent_teal_background].mean()) >= 0.97
    assert float(debug["final_alpha"][subject_holes].mean()) >= 0.80
    assert float(debug["ownership_subject"][subject_holes].mean()) >= 0.98
    assert float(debug["final_alpha"][bright_effect_smoke].mean()) >= 0.55
    assert float(debug["ownership_effect"][bright_effect_smoke].mean()) >= 0.80
...
def test_green_screen_real_frame_3_visible_teal_residue_is_reclaimed_without_damaging_subject_or_lightning():
    ...
    assert float((debug["final_alpha"][cyan_halo_near] > 0.03).mean()) >= 0.90
    assert float(debug["final_alpha"][cyan_halo_near].mean()) >= 0.20
    assert float(debug["ownership_effect"][cyan_halo_near].mean()) >= 0.80


def test_green_screen_real_frame_3_global_visible_teal_residue_rate_is_extremely_low():
    matte = HybridMatte(MattingConfig(use_ai=False))
    matte.last_active_ai_model = "gvm"
    matte.last_fallback_quality_metrics = {
        "score_blocked": True,
    ...
    yellow_or_orange_core = (
        ((red > 200.0) & (green > 155.0) & (blue > 90.0) & (brightness > 165.0))
        | ((red > 210.0) & (green > 120.0) & (green < 190.0) & (blue < 130.0) & (brightness > 145.0))
    )
    bright_lightning_core = (
        (white_core | blue_white_core | yellow_or_orange_core)
        & (~purple_subject)
        & (~screen_green)
    )
    component_count, labels = cv2.connectedComponents(bright_lightning_core.astype(np.uint8), connectivity=8)
    if component_count > 1:
        component_sizes = np.bincount(labels.ravel())
        keep_labels = np.where(component_sizes >= 12)[0]
        keep_labels = keep_labels[keep_labels != 0]
        bright_lightning_core = (
            ...
            if keep_labels.size
            else np.zeros_like(bright_lightning_core)
        )
        bright_lightning_core = cv2.morphologyEx(
            bright_lightning_core.astype(np.uint8),
            cv2.MORPH_CLOSE,
            cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)),
            iterations=1,
        ).astype(bool)
    main_white_blue_lightning_lines = cv2.dilate(
        ((white_core | blue_white_core) & bright_lightning_core).astype(np.uint8),
    ...
    visible_teal_residue_candidate = (
        (blue > 80.0)
        & (green > 120.0)
        & (red < 170.0)
        & (brightness > 80.0)
    ...
    debug = matte.last_green_screen_layer_debug
    visible_residue_soft = visible_teal_residue_candidate & (merged > 0.03)
    visible_residue_hard = visible_teal_residue_candidate & (merged > 0.10)

    assert debug is not None
    assert int(visible_teal_residue_candidate.sum()) >= 500_000
    assert float(visible_residue_soft.mean() / visible_teal_residue_candidate.mean()) <= 0.03
    assert float(visible_residue_hard.mean() / visible_teal_residue_candidate.mean()) <= 0.01
    assert float(debug["ownership_background"][visible_teal_residue_candidate].mean()) >= 0.98
    assert int(subject_central.sum()) >= 100_000
    assert float(debug["final_alpha"][subject_central].mean()) >= 0.80
    assert float(debug["ownership_subject"][subject_central].mean()) >= 0.98
    assert int(bright_lightning_core.sum()) >= 250_000
    assert float(debug["final_alpha"][bright_lightning_core].mean()) >= 0.55
    assert float(debug["ownership_effect"][bright_lightning_core].mean()) >= 0.80
    assert int(main_white_blue_lightning_lines.sum()) >= 200_000
    assert float(debug["final_alpha"][main_white_blue_lightning_lines].mean()) >= 0.55


def test_green_screen_real_frame_3_subject_owned_body_core_is_solid_without_teal_regression():
    matte = HybridMatte(MattingConfig(use_ai=False))
    matte.last_active_ai_model = "gvm"
    matte.last_fallback_quality_metrics = {
        "score_blocked": True,
    ...
    bright_lightning_core = (white_core | blue_white_core | yellow_or_orange_core) & (~purple_subject) & (~screen_green)
    component_count, labels = cv2.connectedComponents(bright_lightning_core.astype(np.uint8), connectivity=8)
    if component_count > 1:
        component_sizes = np.bincount(labels.ravel())
        keep_labels = np.where(component_sizes >= 12)[0]
    ...
    eroded_subject = cv2.erode(
        subject_central.astype(np.uint8),
    ...
    assert debug is not None
    assert int(body_core.sum()) >= 15_000
    assert int(hair_edge.sum()) >= 15_000
    assert float(debug["ownership_subject"][body_core].mean()) >= 0.99
    assert float((base_alpha[body_core] >= 0.95).mean()) <= 0.01
    assert float((ai_alpha[body_core] >= 0.95).mean()) <= 0.01
    assert float((merged[body_core] >= 0.95).mean()) >= 0.90
    assert float(merged[body_core].mean()) >= 0.97
    assert float((merged[hair_edge] < 0.95).mean()) >= 0.20
    assert float((merged[visible_teal_residue] > 0.0).mean()) == 0.0
    assert float((merged[cyan_halo_near] > 0.03).mean()) >= 0.90
    assert float(merged[cyan_halo_near].mean()) >= 0.20
    assert float(debug["ownership_effect"][cyan_halo_near].mean()) >= 0.80
    assert float(debug["final_alpha"][main_white_blue_lightning_lines].mean()) >= 0.55
    assert float(debug["ownership_effect"][main_white_blue_lightning_lines].mean()) >= 0.80
...
def test_green_screen_background_evidence_preserves_cyan_halo_near_non_white_luminous_core():
    ...
    blue_white_core = np.zeros(frame.shape[:2], dtype=bool)
    blue_white_core[8:12, 10:14] = True
    yellow_white_core = np.zeros(frame.shape[:2], dtype=bool)
    yellow_white_core[24:28, 10:14] = True
    ...
    assert debug is not None
    assert float(debug["background_evidence"][blue_white_core | yellow_white_core].mean()) == 0.0
    assert float(debug["ownership_effect"][blue_white_core | yellow_white_core].mean()) >= 0.95
    assert float(debug["final_alpha"][blue_white_core | yellow_white_core].mean()) >= 0.95
    ...
    assert float(merged[near_cyan_halo].mean()) >= 0.19
...
def test_green_screen_gvm_merge_uses_competitive_layer_composer_and_records_debug():
    ...
    assert np.allclose(
        matte.last_green_screen_layer_debug["ownership_subject"],
        np.array([[1.0, 0.0]], dtype=np.float32),
    )
    assert np.allclose(
        matte.last_green_screen_layer_debug["ownership_effect"],
        np.array([[0.0, 1.0]], dtype=np.float32),
    )


def test_green_screen_gvm_merge_keeps_only_last_frame_debug_by_default():
    matte = HybridMatte(MattingConfig(use_ai=False, transparency_preserve=1.0))
    matte.last_active_ai_model = "gvm"
    ...
    ai_alphas = [np.array([[0.90]], dtype=np.float32) for _ in frames]
    zero = np.zeros((1, 1), dtype=np.float32)
    matte._green_screen_subject_confidence = lambda _ai, base, _frame: base
    matte._green_screen_ai_subject_layer = lambda _ai, base, _frame, _gate: base
    matte._green_screen_solid_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_score_blocked_subject_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_subject_integrity_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_semantic_subject_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_effect_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_luminous_effect_reconstruction_layer = lambda *_args, **_kwargs: zero
    matte._green_screen_gvm_fallback_subject_mask = lambda *_args, **_kwargs: np.zeros((1, 1), dtype=bool)
    matte._should_apply_gvm_subject_fallback = lambda *_args, **_kwargs: False
    matte._recover_degenerate_gvm_subject_alpha = lambda alpha, *_args, **_kwargs: alpha

    matte._merge_green_screen_effects(base_alphas, ai_alphas, frames)

    assert isinstance(matte.last_green_screen_layer_debug, dict)
    assert np.allclose(
        matte.last_green_screen_layer_debug["subject_alpha"],
        np.array([[0.50]], dtype=np.float32),
    )
