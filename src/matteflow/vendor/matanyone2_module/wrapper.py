...
def _candidate_checkpoint_dirs() -> List[str]:
    ...
    Order:
      1. ``MATTEFLOW_MATANYONE2_WEIGHTS_DIR`` if set.
      2. ``_BUNDLED_CHECKPOINT_DIR`` beside this vendored module.
      3. ``<vendor_root>/pretrained_models`` as a legacy fallback.
    """
    dirs: List[str] = []
    env_dir = os.environ.get("MATTEFLOW_MATANYONE2_WEIGHTS_DIR")
    if env_dir:
        dirs.append(env_dir)
...
def _ensure_checkpoint(ckpt_path: str | None = None) -> str:
    ...
    # Nothing found — download to the first writable candidate.
