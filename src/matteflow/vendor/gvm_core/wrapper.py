...
def _resolve_gvm_weights_dir():
    ...
    Checks an explicit environment override first, then falls back to the
    bundled directory beside this file so the vendored package stays
    self-contained inside MatteFlow.
    """
    candidates = []
    env_override = os.environ.get("MATTEFLOW_GVM_WEIGHTS_DIR")
    if env_override:
...
class GVMProcessor:
    def __init__(self, 
        ...
        # Vendored GVM stays offline/self-contained: only local weights are allowed.
    ...
    def process_sequence(self, input_path, output_dir,
        ...
        for batch_id, batch in tqdm(enumerate(dataloader), total=total_batches, desc=f"Inferencing {file_name}"):
            ...
            # Preserve the model's soft alpha so glows and semi-transparent details
